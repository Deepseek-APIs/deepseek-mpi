...
`configure` still probes for `libxml2`, `libmagic`, `libarchive`, `poppler-glib`, and `tesseract`. When present, Deepseek MPI can unpack Office/LibreOffice containers (`.docx`, `.xlsx`, `.odt`, `.ods`, etc.) and OCR PDFs (Poppler renders each page to bitmaps and Tesseract performs the recognition), then pass the extracted text into the normal chunking pipeline instead of uploading opaque binaries. The PDF path is now OCR-only—Poppler’s text layer is never used—so results are consistent regardless of whether the original PDF had selectable text. Even without those libraries:
...
This all happens transparently inside `attachment_extract_text_payload`. If you run without `libxml2/libarchive`/`poppler-glib`/`tesseract`, archive/PDF extraction simply falls back to the other heuristics; PDFs won’t be OCR’d and will be treated as opaque binaries (or best-effort text) instead. Set the `TESSERACT_LANG` environment variable (default `eng`) to pick a different OCR language when the OCR stack is available.
