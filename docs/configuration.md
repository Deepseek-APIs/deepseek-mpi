...
`configure` still probes for `libxml2`, `libmagic`, `libarchive`, `poppler-glib`, and `tesseract`. When present, Deepseek MPI can unpack Office/LibreOffice containers (`.docx`, `.xlsx`, `.odt`, `.ods`, etc.) and OCR PDFs (via Poppler + Tesseract), then pass the extracted text into the normal chunking pipeline instead of uploading opaque binaries. Even without those libraries:
...
This all happens transparently inside `attachment_extract_text_payload`. If you run without `libxml2/libarchive`/`poppler-glib`/`tesseract`, archive/PDF extraction simply falls back to the other heuristics; no CLI flags need to change. Set the `TESSERACT_LANG` environment variable (default `eng`) to pick a different OCR language when the OCR stack is available.
